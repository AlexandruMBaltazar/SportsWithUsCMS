...
<?php 
	if(isset($_POST["Submit"])){
		...
		if(empty($title)){
			...
			$editId=$_GET["Edit"];
			$query="UPDATE admin_panel SET title='$title', category='$category', image='$image', post='$post', author='$admin' WHERE id='$editId'";
			...
			if($Execute){
				...
				$_SESSION["errorMessage"]="*Post Couldn't Be Updated!";
				redirect_to("dashboard.php");
			}
		}
	}
		...
		<div class="row">
			...
			<div id="main-content" class="col-lg-10 col-md-12 col-sm-12">
				...
				<div>
					...
					<?php
						$editId=$_GET["Edit"];
						$updateQuery="SELECT * FROM admin_panel WHERE id=$editId";
					...
					<form action="editpost.php?Edit=<?php echo $editId?>" method="post" enctype="multipart/form-data">
						<fieldset>
							<div class="form-group">
								...
								<input class="form-control" id="title" type="text" name="Title" placeholder="Title" value="<?php echo $titleToBeUpdated ?>">
							</div>
							<div class="form-group">
								...
								<select class="form-control" id="category" name="Category">
									<?php
							  			...
							  			$selectQuery="SELECT name FROM category ORDER BY datetime DESC";
							  			$Execute=mysqli_query($Connection, $selectQuery);

							  			while($dataRows=mysqli_fetch_array($Execute)){
							  				$categoryName=$dataRows["name"];

							  				if($categoryName==$categoryToBeUpdated){
												echo "<option selected>".$categoryToBeUpdated."</option>";
											}else{
							...
							<div class="form-group">
								...
								<textarea class="form-control" name="Post" id="postarea" rows="4"> <?php echo $postToBeUpdate?></textarea>
							</div>
							<br>
